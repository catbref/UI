...
class TradePortal extends LitElement {
	...
	constructor() {
		...
		this._foreignBlockchain = 'DOGECOIN'
	}
		...
		return html`
			<div id="trade-portal-page">
				<div style="min-height:48px; display: flex; padding-bottom: 6px; margin: 2px;">
					<h2 style="margin: 0; flex: 1; padding-top: .1em; display: inline;">Trade Portal - QORT/DOGE</h2>
				</div>
							...
							<div class="box">
								...
								<div id="tabs-1-content">
									...
									<div class="card">
										...
										<p>
											<mwc-textfield style="width:100%;" id="buyPriceInput" required readOnly label="Price Ea. (DOGE)" placeholder="0.0000" type="text" auto-validate="false" outlined value="${this.initialAmount}">
											</mwc-textfield>
										</p>
										<p style="margin-bottom: 8px;">
											<mwc-textfield style="width:100%;" id="buyTotalInput" required readOnly label="Total (DOGE)" placeholder="0.0000" type="text" auto-validate="false" outlined value="${this.initialAmount}"> </mwc-textfield>
										...
										<span class="you-have">You have: ${this.dogeBalance} DOGE</span>
									...
									<div id="tab-sell-content">
										<div class="card">
											...
											<p style="margin-bottom: 8px;">
												<mwc-textfield style="width:100%;" id="sellTotalInput" required readOnly label="Total (DOGE)" placeholder="0.0000" type="text" auto-validate="false" outlined value="${this.initialAmount}"> </mwc-textfield>
											</p>
							...
							<div class="box">
								...
								<div class="border-wrapper">
									<vaadin-grid theme="compact column-borders row-stripes wrap-cell-content" id="myOrdersGrid" aria-label="My Orders" .items="${this.myOrders}">
										...
										<vaadin-grid-column resizable header="Amount (QORT)" path="qortAmount"></vaadin-grid-column>
										<vaadin-grid-column resizable header="Total (DOGE)" path="foreignAmount"></vaadin-grid-column>
										<vaadin-grid-column
											resizable
											width="5rem"
											flex-grow="0"
			...
			<mwc-dialog id="manageStuckOrdersDialog" scrimClickAction="${this.cancelStuckOfferBtnDisable ? '' : 'close'}">
				...
				<div>
					<vaadin-grid style="width: 500px" theme="compact column-borders row-stripes wrap-cell-content" id="stuckOrdersGrid" aria-label="My Offering Orders" .items="${this.myOfferingOrders}">
						<vaadin-grid-column resizable header="Amount (QORT)" path="qortAmount"></vaadin-grid-column>
						<vaadin-grid-column resizable header="Price (DOGE)" path="price"></vaadin-grid-column>
						<vaadin-grid-column resizable header="Total (DOGE)" path="expectedForeignAmount"></vaadin-grid-column>
						<vaadin-grid-column
							resizable
							width="5rem"
							flex-grow="0"
	...
	firstUpdated() {
		...
		this.openOrdersGrid.querySelector('#priceColumn').headerRenderer = function (root) {
			root.innerHTML = '<vaadin-grid-sorter path="price" direction="asc">Price (DOGE)</vaadin-grid-sorter>'
		}
	...
	processRedeemedTrade(offer) {
		...
		if (offer.qortalCreator === this.selectedAddress.address) {
			// Check and Update DOGE Wallet Balance
	 ...
	 *   - at this point, Bob's trade-bot finds P2SH-B on the dogecoin blockchain and can send secret-B to P2SH-B
	 ...
	 *   - after Alice redeems QORT from AT, Bob can extract secret-A and capture the actual DOGE funds from P2SH-A to his own dogecoin account and hence his trade-bot moves to BOB_DONE
	 ...
	 *   - so her trade-bot waits for that to appear in dogecoin blockchain, so until then is ALICE_WAITING_FOR_P2SH_A
	...
	async sellAction() {
		...
		const makeRequest = async () => {
			const response = await parentEpml.request('tradeBotCreateRequest', {
				...
				foreignBlockchain: 'DOGECOIN', // TODO: remove hard-coded values
				foreignAmount: parseFloat(sellTotalInput),
				tradeTimeout: 60, // FIX: reduce the tradeTimeout to 1 hour (60 minutes)
				receivingAddress: this.selectedAddress.dogeWallet.address,
			})
	...
	async buyAction() {
		...
		const makeRequest = async () => {
			const response = await parentEpml.request('tradeBotRespondRequest', {
				...
				foreignKey: this.selectedAddress.dogeWallet.derivedMasterPrivateKey,
